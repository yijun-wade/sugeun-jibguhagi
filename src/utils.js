...
// ── 한줄 판단 문장 ──────────────────────────
// tag: DONG[dong].tag 값
export function getVerdict(tag, dong) {
  const dongVerdict = {
    '합정동':   '홍대 인근이라 상권이 풍부하고 2·6호선 환승이 가능한 단지입니다',
    '신도림동': '1·2호선 환승이 가능하고 대형 쇼핑몰이 도보권인 단지입니다',
    '당산동':   '2·9호선 환승역이라 여의도·강남 방향 모두 출퇴근하기 편한 단지입니다',
    '망원동':   '망원한강공원까지 걸어갈 수 있어 일상 여가가 편한 동네입니다',
    '상암동':   'DMC 미디어·IT 직주근접이라 업무지구 출퇴근이 편한 단지입니다',
    '구로동':   '구로디지털단지 직주근접으로 IT 업무지구 통근이 쾌적한 단지입니다',
    '가양동':   '한강 조망이 가능하고 9호선 접근성도 좋은 단지입니다',
    '수유동':   '강북 최대 상권으로 마트·병원·편의시설이 집중된 단지입니다',
    '미아동':   '롯데백화점·대형마트가 도보권에 있어 생활 편의성이 높은 단지입니다',
  }
  if (dong && dongVerdict[dong]) return dongVerdict[dong]
  const map = {
  ...
  // 해당 없으면 null. 전에는 '실거래 데이터 기준 검토해볼 만한 단지 — 상세 탭에서 확인하세요'를 돌려줬는데,
  // 첫 화면 제일 좋은 자리에 아무 말도 아닌 문장이 앉았고 실거래가 없는 임대주택에도 그대로 나갔다.
  return map[tag] || null
}
