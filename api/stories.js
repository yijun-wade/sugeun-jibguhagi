...
async function naverSearch(endpoint, query, display = 6) {
...
export default async function handler(req, res) {
  ...
  try {
    const [b1,b2,b3,c1,c2] = await Promise.all([
      naverSearch(NAVER_BLOG, blogQ[0], 5), naverSearch(NAVER_BLOG, blogQ[1], 5), naverSearch(NAVER_BLOG, blogQ[2], 5),
      naverSearch(NAVER_CAFE, cafeQ[0], 4), naverSearch(NAVER_CAFE, cafeQ[1], 4),
    ])
    const seen = new Set()
    const stories = [...b1,...b2,...b3,...c1,...c2]
      .filter(item => { const t = stripHtml(item.title); if (isCommercial(t)||seen.has(item.link)) return false; seen.add(item.link); return true })
      .slice(0, 8)
      .map(item => ({ title: stripHtml(item.title), description: stripHtml(item.description), link: item.link, source: item.bloggername||item.cafename||'', date: item.postdate||'' }))
