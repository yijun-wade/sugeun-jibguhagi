...
제목 규칙 — 노출을 좌우한다:
뉴스 헤드라인처럼 쓰면 언론사 기사 수천 건과 경쟁해 검색에서 밀린다.
실측(2026-08-27): 경쟁 100건 넘는 제목은 전부 30위 밖, 20건 이하는 노출 100%.
구체적 숫자나 1인칭 경험담 어투를 넣어 고유하게 만들 것.
피할 것 — 세제 개편이 전세난을 부른다
좋은 예 — 전세 알아보다 3천만원 더 부른 이유

[글 형식 — 반드시 이 형식으로만 출력]

[제목]
(제목 규칙을 지킨 제목. 브리핑 제목을 그대로 쓰지 말고 재해석. 25자 이내)
...
다른 설명 없이 위 형식만 출력.`
  ...
  // 빈 초안을 저장하면 다음 실행이 그걸 "이미 있음"으로 보고 건너뛴다.
  // 실제로 프롬프트가 깨져 제목·본문·태그가 전부 빈 파일이 저장됐다(2026-08-29).
  if (!title || !body) {
    throw new Error(`응답 파싱 실패 — 제목 ${title.length}자 / 본문 ${body.length}자. 파일을 만들지 않는다.`)
  }
