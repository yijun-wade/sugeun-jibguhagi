...
// 5종을 두고 매일 4개(슬롯 수)만 뽑는다. 매일 하나씩 쉬는 카테고리가 바뀌어
// 다양성이 올라가고, 유한한 풀(정책 8건)의 소진 속도도 늦춰진다.
export const CATEGORIES = ['브리핑', '용어사전', '정책', '임장가이드', '실거래가분석']

/**
 * 소재의 고유 키. 중복 판정의 유일한 기준 — 뽑을 때도 거를 때도 이걸 쓴다.
 * 공백을 지우는 이유: 사용 이력을 파일명에서 역산하는데 파일명에는 공백이 없다
 * ("용산구 이촌동" → 2026-08-15-용산구이촌동임장가이드.md). 정규화하지 않으면
 * 뽑는 쪽과 거르는 쪽 규칙이 어긋나 같은 소재를 매일 다시 고른다.
 */
export const keyOf = (t) => `${t.category}:${String(t.subject).replace(/\s+/g, '')}`
